...
pub fn header_builder<T: Data>(text: impl Into<LabelText<T>>, command: Command) -> impl Widget<T> {
    let back_button = BACK_BUTTON_ICON
        .to_svg()
        .padding(8.)
        .controller(ClickableArea)
        .on_click(move |ctx: _, _: &mut T, _: &_| ctx.submit_command(command.clone()));
